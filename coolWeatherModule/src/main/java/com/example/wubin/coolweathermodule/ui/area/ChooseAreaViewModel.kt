...
import com.example.wubin.coolweathermodule.data.model.place.City
import com.example.wubin.coolweathermodule.data.model.place.County
import com.example.wubin.coolweathermodule.data.model.place.Province
import com.example.wubin.coolweathermodule.ui.area.ChooseAreaFragment.Companion.LEVEL_CITY
import com.example.wubin.coolweathermodule.ui.area.ChooseAreaFragment.Companion.LEVEL_COUNTY
import com.example.wubin.coolweathermodule.ui.area.ChooseAreaFragment.Companion.LEVEL_PROVINCE
...
class ChooseAreaViewModel(private val repository: PlaceRepository) : ViewModel() {
    ...
    private fun launch(block: suspend () -> Unit) = viewModelScope.launch {
        try {
            ...
            t.printStackTrace()
            Toast.makeText(BaseActivity.myActivity, t.message, Toast.LENGTH_SHORT).show()
            dataChanged.value = dataChanged.value?.plus(1)
            isLoading.value = false
        }
    }
