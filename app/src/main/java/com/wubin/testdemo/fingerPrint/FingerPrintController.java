...
public class FingerPrintController {
    ...
    public FingerPrintController(AppCompatActivity activity) {
    ...
    public boolean supportFingerprint() {
        if (Build.VERSION.SDK_INT < 23) {
            ...
            if (!fingerprintManager.isHardwareDetected()) {
                Toast.makeText(mActivity, "您的手机不支持指纹功能", Toast.LENGTH_SHORT).show();
                return false;
            } else if (!keyguardManager.isKeyguardSecure()) {
                Toast.makeText(mActivity, "您还未设置锁屏，请先设置锁屏并添加一个指纹", Toast.LENGTH_SHORT).show();
                return false;
            } else if (!fingerprintManager.hasEnrolledFingerprints()) {
                Toast.makeText(mActivity, "您至少需要在系统设置中添加一个指纹", Toast.LENGTH_SHORT).show();
                return false;
            }
        }
        return true;
    ...
    private void initKey() {
        try {
            ...
            KeyGenParameterSpec.Builder builder = new KeyGenParameterSpec.Builder(
                    DEFAULT_KEY_NAME, KeyProperties.PURPOSE_ENCRYPT | KeyProperties.PURPOSE_DECRYPT)
