...
public class ActivityUtil {
    ...
    private static void toActivity() {
        ...
        try {
            ...
            Intent intent = new Intent(myActivity, mClass);

            if (mFlag > 0) intent.addFlags(mFlag);

            if (mRequestCode > 0) {
                myActivity.startActivityForResult(intent, mRequestCode);
            } else {
    ...
    public static void checkActivity(String name) throws Exception {
        ...
        if (null == myActivity) throw new MyException(name, "activity 为空");
